...
def kernel(
    ...
    M_t:                 Sampler for the proposal distribution at time t.
...
def forward_pass(
    ...
    """
        ...
        x_star:           Reference trajectory to update.
        b_star:           Indices of the reference trajectory.
        Fs:               The transition matrices for each of the latent components.
        Qs:               The covariance matrices for each of the latent components.
        indices:          (T,) Array of 1-D indexes for which the observation is over.
        M_0:              Sampler for the initial distribution.
                            The first element is the sampling function, taking two arguments (key and number of particle)
                            and the second element is the logpdf taking a single particle as argument.
        Gamma_0:          Initial weight function.
...
def backward_scanning_pass(key, As, b_star_T, xs, log_w_T, ancestor_move_func):
    ...
    inps = tree_map(lambda x: x[-2::-1], xs), As[::-1]
    _, (xs, Bs) = jax.lax.scan(body, B_T, inps)
...
def _marginalise_means(i: Array, u: Array, mu_pred: Array, P_pred: Array):
    """
    Condition full predictive Gaussian on sampled coordinate u = x[i].

    Parameters
    ----------
    i:        Observed coordinate index.
    u:        (N,)   sampled coordinate values.
    mu_pred:  (N, D) predictive means.
    P_pred:   (D, D) predictive covariance.

    Returns
    -------
    mu:       (N, D) conditional means.
    """
    u = jnp.ravel(u)
    m = mu_pred[:, i]
