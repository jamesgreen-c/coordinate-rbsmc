...
from experiments.bayesian.utils import ou_diag_transition, _diag_or_vector_at, _logdiffexp
...
class CorporateBondPrior(Prior):
    ...
    def init(self, key: PRNGKey, params: dict, data, config):
        """
        params: The set of true fixed parameters.
        """
        # set fixed params to variable
        self.params = params

        # init parameters to be optimised
        H, _, llambda, _, tau, _ = Horseshoe.init(self.D)
        chol_H = jnp.linalg.cholesky(H)
        log_llambda = jnp.log(llambda)
        log_tau = jnp.log(tau)

        # return parameters in an unconstrained form
        opt_params = {
            "chol_H": chol_H,
            "log_llambda": log_llambda,
            "log_tau": log_tau
        }
        return opt_params
    ...
    def log_p0(self, params: dict, x0, constant: bool = True):
        ...
        def _logpdf(_z0, _eta0):
            ...
            val += mvn_logpdf(_eta0, m0, None, chol_inv=inv_chol_H0, constant=constant)
            return val
    ...
    def log_ht(self, params, x, data: tuple[Array]):
        """
        ...
        obs_type:  (,) Jax integer in [0, 1, 2] to identify the type of trade observed
        ...
        def case_2():
            ...
            val -= jnp.log(2.0 * alpha_i)  # normalisation
    ...
    def theta_logpdf(self, params):
        
        chol_H = params["chol_H"]
        log_llambda = params["log_llambda"]
        log_tau = params["log_tau"]

        H = chol_H @ chol_H.T
        llambda = jnp.exp(log_llambda)
        tau = jnp.exp(log_tau)

        return Horseshoe.logpdf(H, llambda, tau)
