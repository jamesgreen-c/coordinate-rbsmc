...
# final_A = final_parameters["prior"]["trainable"]["A"]
# best_A = best_parameters["prior"]["trainable"]["A"]
# true_A = data["true_A"]
...
beta = final_parameters["prior"]["trainable"]["beta"]
best_beta = best_parameters["prior"]["trainable"]["beta"]
true_beta = data["true_beta"]
beta_error = np.abs(beta - true_beta).sum()
print("\nEstimated beta: ", beta)
print("\nBest estimated beta: ", best_beta)
print("True beta: ", true_beta)
print("Beta estimation error ", beta_error)

delta = final_parameters["prior"]["trainable"]["delta"]
best_delta = best_parameters["prior"]["trainable"]["delta"]
true_delta = data["true_delta"]
delta_error = np.abs(delta - true_delta).sum()
print("\nEstimated delta: ", delta)
print("\nBest estimated delta: ", best_delta)
print("True delta: ", true_delta)
print("delta estimation error ", delta_error)


# print learnt chol_H versus truth
from experiments.bayesian.prior import _construct_cov_cholesky
est_chol_H = _construct_cov_cholesky(beta, delta)
true_chol_H = _construct_cov_cholesky(true_beta, true_delta)
chol_H_error = np.abs(est_chol_H - true_chol_H).sum()
print("\nEstimated chol H ", est_chol_H)
print("True chol H ", true_chol_H)
print("chol_H estimation error ", chol_H_error)
