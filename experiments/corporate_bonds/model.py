...
def log_potential(
    ...
    z_i:       (,) Jax float of the sampled log half-spread
    eta_i:     (,) Jax float of the sampled mid-YtB
    obs_type:  (,) Jax integer in [0, 1, 2, 3, 4] to identify the type of trade observed
    alpha:     (,) Jax float for alpha (D2D half-width) for the relevant bond index
    psi:       (dim,) Baseline half-spread scale
    chol_R:    (dim,) or (dim, dim) Observation noise standard deviations
